...
def main():
    ...
    unet = SatUNet.from_pretrained(
        ...
        num_metadata=args.num_metadata, use_metadata=args.num_metadata > 0, low_cpu_mem_usage=False,
    )
    ...
    if args.use_ema:
        ...
        #     SatUNet.from_pretrained(
        #     args.pretrained_model_name_or_path, subfolder="unet", revision=args.revision,
        #     num_metadata=args.num_metadata, use_metadata=args.num_metadata > 0, in_channels=unet.conv_in.in_channels,
        #     low_cpu_mem_usage=False,
        # )
        ema_unet = EMAModel(ema_unet.parameters(), model_cls=SatUNet, model_config=ema_unet.config)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            if args.use_ema:
                load_model = EMAModel.from_pretrained(os.path.join(input_dir, "unet_ema"), SatUNet)
            ...
            for i in range(len(models)):
                ...
                load_model = SatUNet.from_pretrained(input_dir, subfolder="unet")
    ...
    def fmow_preprocess_example(example):
        ...
        output = {
            'pixel_values': train_transforms(example['input.npy']),
            'low_res': None,
            'input_ids': input_ids,
    ...
    def spacenet_preprocess_example(example):
        rgb_np = example['rgb.npy']
        if rgb_np.shape[0] == 3:  # for some f-ed up reason channels are fist
            rgb_np = rgb_np.transpose(1, 2, 0)  # c,h,w -> h,w,c

        if isinstance(rgb_np, np.ndarray) and rgb_np.dtype == np.uint16:
            rgb_np = percentile_normalization(rgb_np, axis=(0, 1))

        if args.text_metadata:
            metadata = spacenet_numerical_metadata(example, args.resolution, args.num_metadata, base_year=0)
            input_ids = spacenet_tokenize_caption(example, tokenizer, return_text=True)
            input_ids = combine_text_and_metadata(input_ids, metadata, tokenizer)
        else:
            metadata = spacenet_numerical_metadata(example, args.resolution, args.num_metadata)
            input_ids = spacenet_tokenize_caption(example, tokenizer)
        output = {
            'pixel_values': train_transforms(rgb_np),
            'low_res': None,
            'input_ids': input_ids,
            'metadata': metadata_normalize(metadata),
        }
    ...
    def satlas_preprocess_example(example):
        rgb_np = example['rgb.npy']  # t, h, w, 3
        image_names = example['info.json']['image_names']
        rgb_np = rgb_np[-len(image_names):]

        anchor_image = [name for name in image_names if 'anchor' in name]
        if len(anchor_image) == 0:
            idx = random.choice(range(len(image_names)))
        else:
            idx = image_names.index(anchor_image[0])
        rgb_np = rgb_np[idx]
        image_name = image_names[idx]

        if args.text_metadata:
            metadata = satlas_numerical_metadata(example, image_name, args.resolution, args.num_metadata, base_year=0)
            input_ids = satlas_tokenize_caption(example, tokenizer, return_text=True)
            input_ids = combine_text_and_metadata(input_ids, metadata, tokenizer)
        else:
            metadata = satlas_numerical_metadata(example, image_name, args.resolution, args.num_metadata)
            input_ids = satlas_tokenize_caption(example, tokenizer)
        output = {
            'pixel_values': train_transforms(rgb_np),
            'low_res': None,
            'input_ids': input_ids,
    ...
    def combined_preprocess_train(examples):
        for example in examples:
            if 'fmow' in example['__key__']:
                yield fmow_preprocess_example(example)
            elif 'spacenet' in example['__key__']:
                yield spacenet_preprocess_example(example)
            elif 'satlas' in example['__key__']:
                yield satlas_preprocess_example(example)
            else:
                raise NotImplementedError("oops, wrong key")
    ...
    # train_dataset = SampleEqually(train_datasets_list)
    train_dataset = wds.DataPipeline(
        wds.ResampledShards('::'.join(full_shardlist)),
        # wds.shuffle(100),
        wds.tarfile_to_samples(),
        wds.shuffle(100000, initial=1000),
        wds.decode(),
        combined_preprocess_train,
    )
    ...
    def collate_fn(examples):
        ...
        low_res_list = [example['low_res'] for example in examples]
        if all([im is not None for im in low_res_list]):
            low_res = torch.stack(low_res_list)
            low_res = low_res.to(memory_format=torch.contiguous_format).float()
            dict_of_batches['low_res'] = low_res
    ...
    logger.info(f"  Num examples = I don't care")
    ...
    if args.resume_from_checkpoint:
        ...
        elif args.resume_from_checkpoint == "recent":
            path = "checkpoint-recent" if os.path.isdir(os.path.join(args.output_dir, 'checkpoint-recent')) else None
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                # Get the low-res image for superres
                class_labels = None

                # Get the numerical metadata
                metadata = batch["metadata"]  # (N, num_md)

                # NOTE: this version of dropping metadata is probably not the correct way to do it.
                # The "correct" way would be to 0 out the metadata AFTER the timestep projection,
                # but all models I trained didn't use the "correct" method, and used the version below.
                ...
                if args.num_metadata > 0:
                    model_pred = unet(noisy_latents, timesteps, encoder_hidden_states,
                                      class_labels=class_labels, metadata=metadata).sample
                else:
            ...
            if accelerator.sync_gradients:
                ...
                if args.wandb is not None and accelerator.is_main_process:
                    wandb.log(
                        {"Train/loss": train_loss, "Train/lr": lr_scheduler.get_last_lr()[0], "Train/step": global_step}
                    )
                #################################################################
        ...
        # NOTE: I haven't used this code, but if you want to you can modify the log_validation function to work
        # if accelerator.is_main_process:
